...
#   CROPPED BY PT1 BORDERS, 25KM2 AREA FOR EACH POLYGON
...
#######################################################
# 1. MAKE MAINLAND PORTUGAL HEXAGONS, 25KM2
#######################################################
...
portugal_hex_25 <- sf::st_make_grid(
...
# rename
sf::st_geometry(portugal_hex_25) <- "geometry"

portugal_hex_25_final <- portugal_hex_25 |>
  dplyr::filter(
    !grepl(
      "POINT",
...
ggplot() +
  ...
  geom_sf(data = portugal_hex_25_final, alpha = 0.3) +
  theme_void()

summary(st_area(portugal_hex_25_final))

#     Min.  1st Qu.   Median     Mean  3rd Qu.     Max.
#      175 25000000 25000000 23327568 25000000 25000000

sum(st_area(portugal_hex_25_final)) # 88738068090 [m^2]
sum(st_area(portugal_hex_25_final)) / 1000000 # 88738.07 [km2]
nrow(portugal_hex_25_final) # [1] 3804
names(portugal_hex_25_final) # [1] "geometry" "grid_id"

####################################################################
############# SAVE AS GPKG #########################################

sf::st_write(
  portugal_hex_25_final,
  "./maps/hex_cropped_portugal_25km2.gpkg", driver = "GPKG",
  delete_layer = TRUE # To force overwrite file
)  # Create a geopackage file


############# TEST - LOAD GPKG #####################################
nc <- sf::st_read("./maps/hex_cropped_portugal_25km2.gpkg")
